        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div v-else class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p class="text-gray-900">
                  {{ formatDate(equipmentDetail.CREATIONDATE) }}
                </p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1"
                  >Last Changed By</label
                >
                <p class="text-gray-900">
                  {{ equipmentDetail.LASTCHANGEDBYUSER }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1"
                  >Last Changed On</label
                >
                <p class="text-gray-900">
                  {{ formatDate(equipmentDetail.LASTCHANGEDATE) }}
                </p>
              </div>
            </div>
            ...
            <ul class="divide-y divide-gray-200">
              ...
              >
                ...
                <div class="mt-4 grid grid-cols-2 gap-4 text-sm text-gray-600">
                  ...
                  <div>
                    <p class="mb-1">
                      <span class="font-medium">Last changed by:</span>
                      {{ item.LASTCHANGEDBYUSER }}
                    </p>
                    <p>
                      <span class="font-medium">Last changed on:</span>
...
const fetchEquipment = async () => {
  try {
    ...
    const response = await axios.get(
      `https://dxp24.co.za:8081/proxy/http%3A%2F%2Fvhcalabaci.saportals.com%3A50700%2Fneptune%2Fapi%2Fequipment%2FSearch%3Fsap-client%3D800%26IV_PARAMS%3D%7B%7D%26IV_SORTBY%3D%7B%7D%26IS_PAGING%3D${encodeURIComponent(
        JSON.stringify(pagingParams)
      )}%26IV_SERVER_PAGINATION%3Dtrue/baf7b7e4-d0a6-4467-af19-d00249e1b3af`
    );

    equipment.value = response.data;

    // Update pagination info from response using the correct path
    if (response.data.result?.MS_PAGING) {
      const pagingData = response.data.result.MS_PAGING;
      totalRecords.value = pagingData.TOTALRECORDS;
      totalPages.value = pagingData.TOTALPAGES;
      currentPage.value = pagingData.CURRENTPAGE;
    }
  } catch (err) {
...
const viewEquipmentDetail = async (equipmentId) => {
  ...
  try {
    const response = await axios.get(
      `https://dxp24.co.za:8081/proxy/http%3A%2F%2Fvhcalabaci.saportals.com%3A50700%2Fneptune%2Fapi%2Fequipment%2FRead%3Fsap-client%3D800%26IV_PARAMS%3D%7B%22EQUIPMENT%22%3A%22${equipmentId}%22%7D/baf7b7e4-d0a6-4467-af19-d00249e1b3af`
    );
    equipmentDetail.value = response.data.result.MS_EQUIPMENT;
  } catch (err) {
    detailError.value =
      "Failed to load equipment details. Please try again later.";
